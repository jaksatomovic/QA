    ...
    <div class="d-inline-block">
        ...
        <a @click.prevent="showDeleteModal" href="#"
            ...
            <span class="d-inline-block py-1 px-2">
                <i class="fas fa-trash"></i> {{ __('Delete Question') }}
            </span>
        </a>
        ...
        <base-modal v-show="modalDelete" @close="hideDeleteModal">
            ...
            <template v-slot:body>
                <p class="mb-1">
                    {{ __('Are you sure that want to delete this question?') }}
                </p>
                <label class="modal-label d-block mb-0">
                    <input v-model="sendNotification" type="checkbox" checked=""> {{ __('Send notification about deleted question to author') }}
                </label>
            </template>
...
export default {
    ...
    methods: {
        ...
        close () {
            var url = '/admin/questions/'+this.id;
